...
try:
    ...
    conn_str='database=bludb;hostname=9938aec0-8105-433e-8bf9-0fbb7e483086.c1ogj3sd0tgtu0lqde00.databases.appdomain.cloud;port=32459;protocol=tcpip;\
            uid=zwc16826;pwd=mQKUkUr0TYl3EKto;security=SSL'
...
def display():
    ...
    param = "SELECT category,SUM(amount) as sum FROM expenses WHERE userid = " + str(session['id']) + " GROUP BY category"
    res = ibm_db.exec_immediate(ibm_db_conn, param)
    dictionary = ibm_db.fetch_assoc(res)
    expense_break = {'food':'0',
                     'entertainment':'0',
                     'business':'0',
                     'rent':'0',
                     'EMI':'0',
    ...
    total = 0   
    while dictionary !=False:
        expense_break[dictionary['CATEGORY']] = dictionary['SUM']
        total += int(dictionary['SUM']) 
        dictionary = ibm_db.fetch_assoc(res)
    expense_break['total'] = total
    
    return render_template('display.html' ,expense = expense,expense_break = expense_break)
